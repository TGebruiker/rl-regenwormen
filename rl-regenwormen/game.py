...
class Game(Environment):
    ...
    def states(self):
        ...
        return dict(stone_pos=dict(type='int', shape=(16,), num_values=self.nplayers),
    ...
    def validate(self, action):
        ...
        if cont > 0 and not regenworm and nr != 5:
            return False
        dice_sum = sum([min(dice+1, 5) for dice in dice_state
    ...
    def execute_valid_action(self, action):
        ...
        if cont > 0:
            last_stone = [i for i, stone in enumerate(stone_state)
                          ...
                          and stone[1] == 0][0]
            if last_stone:
                self.state['stone_pos'][last_stone] = 1
                self.state['stone_lock'][last_stone] = 1
            if cont == 1:
    ...
    def execute_invalid_action(self):
        ...
        highest_stone = [i for i, stone in enumerate(stone_state)
                         if stone[0] == 0
                         and stone[1] == 0][-1]
        self.state['stone_pos'][highest_stone] = 0
        self.state['stone_lock'][highest_stone] = 1
        return reward

    def next_player(self):
        for i, stone in enumerate(self.get_stone_state()):
            if stone[0] > 0:
                self.state['stone_pos'] = self.rotate(stone[0])
